...
/proc/get_consolidated_frill_state(junction, west_is_diagonal = FALSE, east_is_diagonal = FALSE)
	var/masked = junction & (4|8|16|128)
	...
	if(west_is_diagonal && !has_east && !has_west && !has_ne && !has_nw)
	...
	if(has_south && has_east && !has_west && !has_ne && !has_nw && !has_se && !has_sw)
		return "frill0"  // True S+E diagonal
	if(has_south && has_west && !has_east && !has_ne && !has_nw && !has_se && !has_sw)
	...
	if(has_east && has_west && !has_ne && !has_nw)
		// But if side neighbors are diagonal, show as frill0
		if(west_is_diagonal && east_is_diagonal)
			return "frill0"
		if(west_is_diagonal)
	...
	if(has_east && has_west && has_ne && !has_nw)
		if(east_is_diagonal && !west_is_diagonal && has_se)
			return "frill9"
		return "frill4"
	...
	if(has_west && has_nw && !has_east && !has_ne)
		if(west_is_diagonal && !east_is_diagonal && has_nw)
			return "frill8"
		return "frill6"
	return "frill1"
...
/proc/get_consolidated_front_frill_state(junction, west_is_diagonal = FALSE, east_is_diagonal = FALSE)
	...
	if(has_south && has_east && !has_west && !has_se && !has_sw)
		if(east_is_diagonal)
			return "frill0"  // S+E but east neighbor is diagonal
	if(has_south && has_west && !has_east && !has_se && !has_sw)
		if(west_is_diagonal)
			return "frill0"  // S+W but west neighbor is diagonal
	if(!has_east && !has_west)
		return "frill0"

	if(has_east && has_west)
		if(west_is_diagonal && has_se && !east_is_diagonal)
			return "frill9"  // Slash-up with proper SE corner
		if(east_is_diagonal && has_se && !west_is_diagonal)
			return "frill9"  // Backslash with proper SE corner
		// Otherwise, both neighbors are connected, show as frill1
		return "frill1"
	// frill2: Left only
...
/datum/element/frill
	...
	id_arg_index = 2
	var/icon_path
	var/list/tracked_overlays = list() // target -> list(overlays)
	var/last_south_turf = null  // Track south neighbor to preserve front frills on north changes
	...
	*/
	...
	if(!has_north_wall)
		if(vertical_door_south)
			...
			coverup.pixel_y = 32  // Lower position for door/window coverage
