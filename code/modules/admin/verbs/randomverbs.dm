	...
	if(usr)
		if (usr.client)
			if(usr.client.holder)
				SEND_SOUND(usr.client, 'sound/blank.ogg')
				M.playsound_local(soundin = 'sound/blank.ogg', vol = 100)
...
/client/proc/respawn_character()
	...
	if(is_unassigned_job(new_character.mind.assigned_role))
		new_character.mind.set_assigned_role(/datum/job/kaizoku/wanderer) // Stonekeep edit
