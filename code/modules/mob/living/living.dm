	...
	if((L.body_position != LYING_DOWN) && (body_position != LYING_DOWN)) //we are both standing
		if(I)
			if(I.wlength > WLENGTH_NORMAL)
				...
				//Stonekeep Edit: Groin modification - Start
			...
			//Stonekeep Edit: Groin modification - End
...
/mob/living/proc/mob_sleep()
	...
	if(IsSleeping())
		...
		if(alert(src, "You sure you want to sleep for a while?", "Sleep", "Yes", "No") == "Yes")
			...
			schedule_wilderness_sleep_ambush_if_unprayed() //Stonekeep To:Do; If Prayer before sleep, give spiritual immunity to Yokais and Spiritual fuckery
