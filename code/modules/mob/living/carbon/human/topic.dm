...
/mob/living/carbon/human/Topic(href, href_list)
	...
	if(href_list["embedded_object"] && usr.can_perform_action(src, FORBID_TELEKINESIS_REACH))
		...
		if(do_after(usr, time_taken, src))
			...
			usr.put_in_hands(I)
			emote("pain", TRUE)
			playsound(src, 'modular/kaizoku/sound/foley/flesh_rem.ogg', 100, TRUE, -2)
			if(usr == src)
	...
	if(href_list["undiesthing"]) //canUseTopic check for this is handled by mob/Topic()
		if(!get_location_accessible(src, BODY_ZONE_PRECISE_GROIN, skipundies = TRUE)) // STONEKEEP EDIT: KAIZOKU; GROIN IS ITS OWN BODYPART.
			to_chat(usr, span_warning("I can't reach that! Something is covering it."))
			return
		if(underwear == "Nude")
			return
