...
/mob/living/carbon/human/attack_hand_secondary(mob/user, params)
	...
	if(ishuman(src) && ishuman(user))
		...
		if(length(user.return_apprentices()) >= user.return_max_apprentices())
			return
		if((job?.type == /datum/job/kaizoku/lowlife) && target.mind && !target.is_apprentice()) //Stonekeep Edit.
...
/mob/living/proc/steal_action(atom/A)
	...
	if(ishuman(A))
		...
		var/list/stealablezones = list(BODY_ZONE_CHEST, BODY_ZONE_L_ARM, BODY_ZONE_R_ARM, BODY_ZONE_PRECISE_NECK, BODY_ZONE_PRECISE_GROIN, BODY_ZONE_L_LEG, BODY_ZONE_R_LEG, BODY_ZONE_PRECISE_R_HAND, BODY_ZONE_PRECISE_L_HAND) // STONEKEEP EDIT: KAIZOKU; GROIN IS ITS OWN BODYPART.
		...
		if(stealroll >= target_perception)
			...
			if(do_after(U, (2.5 - (thief_skill_base * 0.25)) SECONDS, V, progress = FALSE))
				switch(U.zone_selected)
					...
					if(BODY_ZONE_PRECISE_GROIN) // STONEKEEP EDIT: KAIZOKU; GROIN IS ITS OWN BODYPART.
...
/mob/living/proc/jump_action_resolve(atom/A, jadded, jrange, jextra)
	...
	if(adjust_stamina(min(jadded,100)))
		...
		if(jextra)
			...
			while(src.throwing)
				if(GrabThatRopeMan()) //Stonekeep: Kaizoku edit. Major portion starts here
					break
				sleep(1)
			if(held_rope)
				return
			...
			while(src.throwing)
				...
				sleep(1)
			if(held_rope)
				return //Stonekeep: Kaizoku edit; Major portion ends here
		else
			throw_at(A, jrange, 1, src, spin = FALSE)
			while(src.throwing)
				if(GrabThatRopeMan()) //Stonekeep edit.
					break
				sleep(1)
			if(held_rope)
				return //End of Stonekeep edit on this path.
