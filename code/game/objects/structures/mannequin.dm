...
/obj/structure/mannequin/proc/EquippableSlots()
	...
	. += "<BR><B>Belt:</B> <A href='byond://?src=[REF(src)];command=item_placement;item_slot=[BODY_ZONE_PRECISE_GROIN]'>[makeStrippingButton(clothing[SLOT_MANNEQUIN_BELT])]</A>" //Stonekeep Edit
...
/obj/structure/mannequin/proc/TipOver()
	playsound(src, 'modular/kaizoku/sound/foley/dropsound/wooden_drop.ogg', 50, TRUE)
	density = FALSE
	tipped_over = TRUE
	DropAll()
...
/obj/structure/mannequin/proc/StandUp(mob/living/L)
	...
	for(var/obj/structure/mannequin/O in get_turf(src))
		if(O.density && !O.tipped_over)
			return
	playsound(src, 'modular/kaizoku/sound/foley/woodclimb.ogg', 50, TRUE)
	density = TRUE
	tipped_over = FALSE
	var/matrix/mat = transform
	transform = mat.Turn(-90)
