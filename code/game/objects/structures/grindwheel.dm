...
/obj/structure/grindwheel/attackby(obj/item/I, mob/living/user, params)
	if(I.max_blade_int)
		playsound(src,'modular/kaizoku/sound/foley/grindblade.ogg', 100, FALSE)
	...
	if(!has_world_trait(/datum/world_trait/delver))
		if(istype(I, /obj/item/grown/log/tree/small))
			...
			playsound(src, pick('modular/kaizoku/sound/misc/slide_wood (2).ogg', 'modular/kaizoku/sound/misc/slide_wood (1).ogg'), 100, FALSE)
