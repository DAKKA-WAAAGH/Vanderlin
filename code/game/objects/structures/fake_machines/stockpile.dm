...
/datum/withdraw_tab/proc/perform_action(href, href_list)
	if(href_list["withdraw"])
		...
		if(D.get_held_count() <= 0)
			...
			playsound(parent_structure, 'modular/kaizoku/sound/misc/hiss.ogg', 100, FALSE, -1)
		return TRUE
...
/datum/withdraw_tab/proc/insert_coins(obj/item/coin/C)
	...
	qdel(C)
	playsound(parent_structure, 'modular/kaizoku/sound/misc/coininsert.ogg', 100, TRUE, -1)
