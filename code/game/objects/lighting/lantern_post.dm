...
/obj/machinery/light/fueled/lanternpost/attack_hand(mob/user)
	...
	if(torchy && !permanent)
		...
		update()
		update_appearance(UPDATE_ICON_STATE)
		playsound(src, 'modular/kaizoku/sound/foley/torchfixturetake.ogg', 100)
	...
	if(istype(W, /obj/item/flashlight/flare/torch))
		...
		if(torchy)
			...
			if(LR.on)
				...
				torchy = LR
				update_appearance(UPDATE_ICON_STATE)
			playsound(src, 'modular/kaizoku/sound/foley/torchfixtureput.ogg', 100)
		return
	if(istype(W, /obj/item/rope)&&!istype(W, /obj/item/rope/chain))
		if(!torchy)
			...
			if(do_after(user, 2 SECONDS, src))
				...
				playsound(src, 'modular/kaizoku/sound/foley/noose_idle.ogg', 100)
				qdel(W)
				qdel(src)
		else
			if(torchy && !permanent)
